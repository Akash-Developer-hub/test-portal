...
const Mcq_Assesment = () => {
  ...
  const saveDataToLocalStorage = (contestId) => {
    try {
      // Prepare the complete contest data
      const payload = {
        contestId,
        assessmentOverview: formData.assessmentOverview,
        testConfiguration: formData.testConfiguration,
        sectionDetails: formData.sectionDetails,  // Include sectionDetails in the same payload
      };
  
      // Get existing contests or initialize empty array
      const existingContests = JSON.parse(localStorage.getItem('contests') || '[]');
      
      // Check if contest already exists
      const existingContestIndex = existingContests.findIndex(contest => contest.contestId === contestId);
      
      if (existingContestIndex > -1) {
        // If contest exists, update it
        existingContests[existingContestIndex] = payload;
        console.log("Contest updated successfully with Contest ID:", contestId);
      } else {
        // If contest doesn't exist, add a new contest
        existingContests.push(payload);
        console.log("Contest added successfully with Contest ID:", contestId);
      }
  ...
  const nextStep = async () => {
    if (validateStep()) {
      // Save the data to localStorage when clicking "Next" on Step 2 or beyond
      if (currentStep === 2) {
        const generatedContestId = Math.random().toString(36).substr(2, 9);
        setContestId(generatedContestId);
    
        try {
          const savedSuccessfully = saveDataToLocalStorage(generatedContestId);
    
          if (savedSuccessfully) {
            // Generate a simple token (in practice, you might want a more secure method)
            const token = btoa(`contest-${generatedContestId}-${Date.now()}`);
            localStorage.setItem("contestToken", token);
            localStorage.setItem("currentContestId", generatedContestId);
            
            navigate("/mcq/sectionDetails");
          } else {
            alert("Failed to save contest data. Please try again.");
          }
        } catch (error) {
          console.error("Error processing contest:", error);
          alert("Failed to process the contest. Please try again.");
        }
        return;
      }
    
      // Move to next step if it's not step 2
      if (currentStep < 3) {
  ...
  // Optional: Function to retrieve contest data
  const getContestData = (contestId) => {
    try {
      ...
      console.error("Error retrieving contest data:", error);
      return null;
    }
  };
